...
export default function Quality() {
  ...
  return (
    <div className="pt-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-7">
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4 bg-background border border-border p-6 rounded-card shadow-card hover:shadow-card-hover transition-all duration-300 shadow-sm">
                <div className="flex items-start gap-3">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />
                  <div>
                    <h4 className="font-semibold text-heading text-xs sm:text-sm">Double Batch Testing</h4>
                    <p className="text-body text-xs mt-0.5">We test raw ingredients both at source and pre-packaging.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0" />
                  <div>
                    <h4 className="font-semibold text-heading text-xs sm:text-sm">Impeccable Traceability</h4>
                    <p className="text-body text-xs mt-0.5">Full computerized trace index for raw materials.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5">
              <div className="bg-primary text-muted rounded-btn shadow-btn hover:shadow-btn-hover transition-all duration-300 shadow-sm p-8 relative overflow-hidden text-center border border-primary">
                <Award className="w-12 h-12 text-white mx-auto mb-4" />
                <h3 className="font-display font-bold text-white text-base">WHO-GMP Recertified</h3>
                <p className="text-[10px] text-body font-mono mt-1 uppercase tracking-wider">Allied Production Hubs</p>
                ...
                <div className="mt-6 pt-6 border-t border-primary grid grid-cols-3 gap-2 text-center text-[11px] font-mono">
                  <div>
                    <span className="block font-bold text-white">ISO 9001</span>
                    <span className="text-[9px] text-body">Standards</span>
                  </div>
                  <div>
                    <span className="block font-bold text-white">COA</span>
                    <span className="text-[9px] text-body">Verified</span>
                  </div>
                  <div>
                    <span className="block font-bold text-white">ICH Q1-Q10</span>
                    <span className="text-[9px] text-body">Compliant</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="bg-primary text-white py-12 border-t border-b border-primary">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center">
            {qualityStats.map((stat, i) => (
              <div key={i}>
                ...
                <span className="block text-[10px] font-mono uppercase text-body mt-1 tracking-wider">
                  {stat.label}
                </span>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {qualityPillars.map((pillar, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-white border border-border p-6 sm:p-8 rounded-card shadow-card hover:shadow-card-hover transition-all duration-300 shadow-sm shadow-sm text-left flex gap-5 items-start"
                >
                  <div className="w-10 h-10 rounded bg-alt-bg text-heading flex items-center justify-center shrink-0">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-left">
          ...
          <h2 className="text-2xl sm:text-3xl font-display font-medium text-heading tracking-tight leading-tight">
          ...
          <div className="mt-8 bg-background border border-border rounded-card shadow-card hover:shadow-card-hover transition-all duration-300 shadow-sm p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h4 className="font-semibold text-heading text-sm">Suspected Drug Side Effect?</h4>
              <p className="text-body text-xs mt-1">Submit a formal report to our Drug Safety desk immediately.</p>
            </div>
            <a
              href="#contact"
              className="px-5 py-2.5 bg-primary hover:bg-primary text-white font-medium text-xs sm:text-sm rounded border border-primary transition-colors text-center shrink-0 cursor-pointer"
            >
